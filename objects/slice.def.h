...
DEFINE MAKE_SLICE_4__(start_, stop_, step_, len) {.class__ = &TYPE_NAME(CLASS_MANGLE(Slice)), .start = start_, .stop = stop_, .step = step_}
...
DEFINE MAKE_SLICE_ITERATOR_4__(start_, stop_, step_, len) {.class__ = &TYPE_NAME(CLASS_MANGLE(SliceIterator)), .sl = MAKE_SLICE_4__(start_, step_, step_, len), .loc = start_, .stop = ITERATOR_PAUSE}
